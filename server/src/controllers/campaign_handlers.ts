 ...
 */
async function insertParticipantsToCampaign(campaignId: string, req: Request, poolClient: PoolClient) {
  // Check if there is no file in the request or not
  if (!req.file) return false;
  const phoneNumbers = req.file.buffer.toString().split(",");
